...
suite('swagger converts', s => {
  ...
  function simpleTest(input, output, components, only) {
    s[only ? 'only' : 'test']('Set ' + i++, t => {
      ...
      if (components)
        t.deepEqual(result.components, components, 'components match');
      t.end();
    });
  }
  ...
  simpleTest(joi.string().label('test'), {
    type: 'string',
    title: 'test',
  });

  simpleTest(joi.string().regex(/^A$/), {
    type: 'string',
    pattern: '^A$',
  });
  ...
  simpleTest(joi.string().only('A', 'B', 'C', null), {
    type: 'string',
    enum: ['A', 'B', 'C'],
    nullable: true,
  });
  ...
  simpleTest(joi.string().example('sii'), {
    example: 'sii',
    type: 'string',
  });
  ...
  simpleTest(joi.date().default(Date.now, 'current date'), {
    type: 'string',
    format: 'date-time',
  });
  ...
  simpleTest(
    joi.object().keys({
      a: joi.object().keys({d: joi.string()}),
      b: joi.string(),
      c: joi.lazy(() => 'a'),
    }),
    {
      type: 'object',
      properties: {
        a: {type: 'object', properties: {d: {type: 'string'}}},
        b: {type: 'string'},
        c: {type: 'a'},
      },
    },
